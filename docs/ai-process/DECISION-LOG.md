...
| 12.09. | Copy-on-first-write gebaut, aber nur für Zugriffe, die etwas anlegen | Eine Quelle oder ein Report macht das Notizbuch zu einem eigenen; eine Frage nicht, sonst entsteht ein Notizbuch je Besucher, der einmal etwas wissen wollte | `notebooks/internal/copy-on-write.ts` |
| 12.09. | "Nochmal versuchen" kopiert nicht | Es braucht die Zeile, um die es geht, und im Demo-Notizbuch gibt es keine: die Kopie wäre leer und die Antwort danach ein 404 | `notebooks.service.ts` (`writable` gegen `writableOrCopy`) |
| 12.09. | Die Route gibt die Notizbuch-Id in der Antwort zurück | Der Server entscheidet, wo ein Schreibzugriff landet; die Oberfläche vergleicht und wechselt, statt es zu erraten | `source.dto.ts`, `studio.dto.ts` |
